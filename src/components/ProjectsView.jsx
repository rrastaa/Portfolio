      ...
      <div className="flex flex-col md:flex-row gap-4 md:gap-8 bg-linear-to-br from-[#BFBFBF]/8 to-[#595959]/5 p-2 md:p-8 rounded-lg border border-white/10">
        ...
        <div className="flex flex-col gap-4 md:gap-8">
          <div className="flex flex-row items-center gap-3">
            <img src={logo} className="max-w-8 max-h-8 md:max-w-14 md:max-h-10" alt="" />
            <div className="flex flex-col">
              <h3 className="text-xl md:text-2xl font-medium">{title}</h3>
              <p className="font-normal text-xs md:text-sm"> {focus}</p>
            </div>
          </div>
          <p className="text-justify text-xs md:text-base font-normal">{desc}</p>
